...
export default async function HrRootPage() {
  ...
  if (!session?.user) {
    redirect(
      `${process.env.NEXT_PUBLIC_BASE_URL}/login?callbackUrl=${encodeURIComponent("https://hr." + (process.env.NEXT_PUBLIC_ROOT_DOMAIN || "angri.nl"))}`,
    );
  }
  ...
  return (
    <PageWrapper>
      <PageHeader
        ...
        description="Choose an email account to manage with the HR Manager."
      />
